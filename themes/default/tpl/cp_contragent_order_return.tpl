	...
	foreach ($_SESSION['errm'] as $msg){if (!is_array($msg)){?>
        ...
        <?}}}unset($_SESSION['errm'])?>
			...
			<form action="<?=$_SERVER['REQUEST_URI']?>" method="post" id="returnForm">
                ...
                <?if ($i['id_return_status']==1){?>
                		<input type="submit" name="smb_return" value="Выполнить возврат">
                	<?}?>
			</form>
                ...
                <br>
<a href="<?=_base_url?>/invoice_contragent_ret/<?=$order['id_order']?>/<?=$order['skey']?>">Возврат по накладной контрагента</a>
            </div><!--class="cabinet"-->