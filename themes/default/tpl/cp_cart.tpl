...
<?}else{?>
	...
	<div class="cart_buttons">
		...
		<input type="hidden" claass="current_user" value="<?=$_SESSION['member']['gid']?>">
		<?$min_sum_order = $_SESSION['cart']['products_sum'][3] >= $GLOBALS['CONFIG']['min_sum_order'];?>
			...
			<?if(!G::IsLogged() || !_acl::isAdmin()){?> <!-- когда клиент просто оформляет заказ-->
				<div id="button-cart1" <?=isset($_SESSION['cart']['promo'])?'class="hidden"':null;?>>
					<button class="mdl-button mdl-js-button mdl-button--raised mdl-button--accent make_order_tag <?=$min_sum_order === false && $_SESSION['member']['gid'] != _ACL_CONTRAGENT_?'hidden':null?>" type='submit' value="Отправить">Оформить заказ</button>
					<p <?=$_SESSION['member']['gid'] == _ACL_CONTRAGENT_ || $min_sum_order === true?'class="hidden"':null?>>Минимальная сумма заказа <?=$GLOBALS['CONFIG']['min_sum_order']?> грн.</p>
				</div>
			<?}else{?>
