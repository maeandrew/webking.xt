			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="list">
				...
				<tr class="filter">
						...
						<td><select name="id_order_status" style="width:60px;" onchange="this.form.submit();">
							...
							<?foreach($order_statuses as $k=>$item){print_r($k);?>
								<option <?=($item['id_order_status']==$_POST['id_order_status'])?'selected="true"':null?> value="<?=$item['id_order_status']?>"><?=$item['name']?></option>
					 		<?}?>
							</select>
						</td>
						<td><input type="text" style="width: 120px;" value="<?=isset($_POST['filter_contragent_name'])?htmlspecialchars($_POST['filter_contragent_name']):null?>" name="filter_contragent_name" /></td>
				...
				<?if(isset($list)){$tigra=false;foreach ($list as $i){?>
					<tr<?if ($tigra==true){?> style="background-color:#f3f3f3;"<?$tigra=false;}else{$tigra=true;}?>>
						<td><?=date("d.m.Y",$i['target_date'])?></td>
						<td><a href="<?=_base_url.'/adm/order/'.$i['id_order']?>"><?=$i['id_order']?>-заказ</a></td>
						...
						<td><a target="_blank" href="<?=_base_url?>/invoice_customer/<?=$i['id_order']?>/<?=$i['skey']?>"><?=$i['sum_discount']?></a></td>
						<td><a target="_blank" href="<?=_base_url?>/invoice_contragent/<?=$i['id_order']?>/<?=$i['skey']?>"><center><?=$i['otpusk_prices_sum']?></center></a></td>

						<td class="center np"><?if($i['id_pretense_status']==0){?><p>&mdash;</p><?}else{?>
							<p><a href="<?=_base_url?>/adm/order/<?=$i['id_order']?>"/><?=$i['id_order']?>-прет</p><?}?></td>

						<td class="center np"><?if($i['id_pretense_status']==0){?><p>&mdash;</p><?}else{?>
							<p<?=($i['id_pretense_status']==2)?' class="status_done"':null?>><?=$order_statuses[$i['id_pretense_status']]['name']?></p><?}?></td>


						<td class="center np"><?if($i['id_return_status']==0){?><p>&mdash;</p><?}else{?>
							<p><a href="<?=_base_url?>/adm/order_return/<?=$i['id_order']?>"/><?=$i['id_order']?>-возв</p><?}?></td>

						<td class="center np"><?if($i['id_return_status']==0){?><p>&mdash;</p><?}else{?>
							<p<?=($i['id_return_status']==2)?' class="status_done"':null?>><?=$order_statuses[$i['id_return_status']]['name']?></p><?}?></td>
