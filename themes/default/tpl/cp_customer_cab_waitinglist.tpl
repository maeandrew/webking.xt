						...
						<div class="waiting_list_js tableRow" data-idproduct="<?=$p['id_product']?>">
							<div class="image_cell">
								<div class="btn_js" data-name="big_photo">
									<?if(!empty($p['img_1'])){?>
										<img class="toBigPhoto" id="big_photo_<?=$p['id_product']?>" alt="<?=G::CropString($p['name'])?>" src="<?=_base_url?><?=G::GetImageUrl($p['img_1'], 'thumb')?>" data-original-photo="<?=_base_url?><?=G::GetImageUrl($p['img_1'])?>">
									<?}else if(!empty($p['images'])){?>
										<img class="toBigPhoto" id="big_photo_<?=$p['id_product']?>" alt="<?=G::CropString($p['name'])?>" src="<?=_base_url?><?=G::GetImageUrl($p['images'][0]['src'], 'thumb')?>" data-original-photo="<?=_base_url?><?=G::GetImageUrl($p['images'][0]['src'])?>">
									<?}else{?>
