<div class="agents_clients_list">
	...
	<?if(empty($agent_users)){?>
		<h2>На данный момент у Вас нет клиентов</h2>
	<?}else{
		$i=1;
		foreach($agent_users as $client){?>
			<div class="agents_client <?=$client['countable'] == 1?null:'disabled'?>">
				...
				<div class="client_info phone "><span class="agent_mobile_label">Тел.:</span> <?=$client['countable'] == 1?'<a href="tel:'.$client['phone'].'">+'.$client['phone'].'</a>':null?></div>
