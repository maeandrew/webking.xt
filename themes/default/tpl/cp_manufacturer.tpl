<rh1>Производитель <?=$manname?></rh1>
	<img src="<?=file_exists($GLOBALS['PATH_root'].$man_image)?_base_url.$man_image:'/images/nofoto.png'?>" class="man_logo">
	<div class="mancats">
			...
			<div class="plate">
				<div class="product<?=($ii==3)?' nm':null?>">
                    <div class="wrap break">
                        <rh4><a class="alt" href="<?=_base_url.'/item/'.$i['item_id'].'/'.$i['translit']?>/"><img src="<?=file_exists($GLOBALS['PATH_root'].$i['img_1'])?_base_url.htmlspecialchars(str_replace("/efiles/", "/efiles/_thumb/", $i['i_image'])):'/images/nofoto.png'?>"><?=$i['name']?></a></rh4>
                        <span class="descr">
                            <?=$i['descr']?>
                        </span>
                        <div class="advan">
