		...
		foreach ($pops as $i){?>
			<li>
				<a href="<?=_base_url."/product/".$i['id_product']."/".$i['translit']."/";?>">
					<img height="180" alt="<?=str_replace('"', '', $i['name'])?>" src="<?=file_exists($GLOBALS['PATH_root'].$i['img_1'])?_base_url.htmlspecialchars(str_replace("/efiles/image/", "/efiles/image/500/", $i['img_1'])):'/images/nofoto.png'?>"/>
					<span><?=$i['name']?></span>
