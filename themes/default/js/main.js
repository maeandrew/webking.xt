...
*/
	...
	//Выбор товара в модальном окне
	$('.gift_modal').on('click','.gift_product_js', function(){
		if($(this).hasClass('active')){
			$('.gift_modal .gift_product_js').removeClass('disabled').removeClass('active');
			$('.id_gift_input_js').val(null);
		}else{
			$('.gift_modal .gift_product_js').removeClass('active').addClass('disabled');
...
var Shares = {
	...
	share: function(id, url, title, desc) {		
		...
		url = url.replace("partner", "share");
		desc = encodeURIComponent(desc);
		title = encodeURIComponent(title);
		switch (id) {
			case 'fb':
				url = url.replace("link", "facebook");
				this.popupCenter('https://www.facebook.com/sharer/sharer.php?u=' + url, this.title, this.width, this.height);
				break;
			case 'vk':
				url = url.replace("link", "vk");
				this.popupCenter('http://sss.cezo.legsq.1.gsr.anonimizing.com/share.php?url=' + url + '&description=' + title + '. ' + desc, this.title, this.width, this.height);
				break;
			case 'tw':
				url = url.replace("link", "twitter");
				var text = title || desc || '';
				...
				break;
			case 'pin':
				url = url.replace("link", "pinterest");
				this.popupCenter('https://pinterest.com/pin/create/button/?url=' + url, this.title, this.width, this.height);
				break;
			case 'viber':
				url = url.replace("link", "viber");
				this.popupCenter('viber://forward?text=' + url, this.title, this.width, this.height);
				break;				
			case 'ok':
				url = url.replace("link", "ok");
				this.popupCenter('http://sss.ceso.cevy.1.gsr.anonimizing.com?st.shareUrl=' + url, this.title, this.width, this.height);
				break;
			case 'WhatsApp':
				url = url.replace("link", "WhatsApp");
				this.popupCenter('whatsapp://send?text=' + url, this.title, this.width, this.height);
				break;
			case 'Telegram':
				url = url.replace("link", "Telegram");
				this.popupCenter('tg://msg?text=' + url, this.title, this.width, this.height);
				break;
			case 'Skype':
				url = url.replace("link", "Skype");
				this.popupCenter('skype:NZeraF?chat=' + url, this.title, this.width, this.height);
				break;
		}
	},
	newTab: function(url) {
