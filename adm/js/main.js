$(function(){
	...
	$('body').on('click', '.adm_comment_reply_js', function(event){
		...
		$(this).closest('table').find('tbody').append('<tr class="new_comment"><td colspan="2"><div class="reply_wrap"><form action="/adm/coment/" method="post" onsubmit="onCommentSubmit()"><input type="hidden" name="pid_comment" value="'+$(this).attr('data-idComment')+'"><input type="hidden" name="id_product" value="'+$(this).attr('data-idproduct')+'"><textarea name="feedback_text" id="feedback_comment_reply" cols="30" required></textarea><div class="user_data hidden"><div class="fild_wrapp"><label for="feedback_author">Ваше имя:</label><input type="text" name="feedback_author" id="feedback_author" required value="'+$(this).attr('data-username')+'"></div><div class="fild_wrapp"><label for="feedback_authors_email">Эл.почта:</label><input type="email" name="feedback_authors_email" id="feedback_authors_email" required value="'+$(this).attr('data-useremail')+'"></div></div><button type="submit" name="sub_com" class="btn-m-green">Ответить</button></form></div></td></tr>');
	});
