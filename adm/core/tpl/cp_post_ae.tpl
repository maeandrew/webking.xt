...
<div id="postae">
    <form action="<?=$GLOBALS['URL_request']?>" method="post">
		...
		<textarea name="content_preview" id="content_preview" class="input-l" rows="18" cols="195"><?=isset($_POST['content_preview'])?htmlspecialchars($_POST['content_preview']):null?></textarea>
		<p><b>Полное описание:</b></p><?=isset($errm['content'])?"<span class=\"errmsg\"><br>".$errm['content']."</span>":null?>
		<textarea name="content" id="content" rows="38" cols="200" class="hidden"><?=isset($_POST['content'])?htmlspecialchars($_POST['content']):null?></textarea>
		...
		<input type="text" name="date" id="date" class="input-l wa" value="<?=(isset($_POST['date']) && !isset($errm['date']))?$_POST['date']:time();?>"/>
