			...
			<?foreach($list as $i){?>
				<tr class="animate">
					...
					<td class="left actions"><nobr>
						<a class="btn-l-green-inv" href="/adm/pageedit/<?=$i['id_page']?>">редактировать</a>
						<a class="btn-l-green-inv" href="/page/<?=$i['translit']?>/">смотреть</a>
						<a class="btn-l-red-inv" href="/adm/pagedel/<?=$i['id_page']?>" onclick="return confirm('Точно удалить?');">удалить</a>
						</nobr>
					</td>
				</tr>
			<?}?>
