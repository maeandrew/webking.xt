					...
					<thead>
						<tr class="coment<?=$i['Id_coment']?> animate <?if(!$i['visible'] && $interval->format('%a') < 3){?>bg-lyellow<?}?>">
							<th class="left">
								...
								<div class="btn_wrap">
									<a class="btn-m-green btn_answer adm_comment_reply_js" href="#" data-idComment="<?=$i['Id_coment']?>" data-username="<?=$i['username']?>" data-idproduct="<?=$i['url_coment']?>" data-useremail="<?=$_SESSION['member']['email']?>">Ответить</a>
