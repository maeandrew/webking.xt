...
$GLOBALS['IERA_LINKS'][$ii]['title'] = 'Редактирование транспортной компании';
$tpl->Assign('h1', $header);
if(!$data = $Address->GetShippingCompanyById($id)) die("Ошибка при извлечении полей");
if(isset($_POST['smb'])){
	require_once($GLOBALS['PATH_block'].'t_fnc.php'); // для ф-ции проверки формы
	list($err, $errm) = Shipping_companies_form_validate();
	if(!$err){
		if($Address->UpdateShippingCompany($_POST)){
