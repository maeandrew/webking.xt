...
if(isset($_POST['parse_URL'])){
	...
	switch ($_POST['site']){
		case 21:
			...
			//Выборка цен для обновления просмотреть список  кодов товара и цен
			 // 	$array_offer  =  array();
			 // 	echo "UPDATE xt_assortiment SET  product_limit = 0, active = 0 WHERE id_supplier = 30939;", "<br />";
				// foreach ($sim_url->xpath('/yml_catalog/shop') as $element) {
				// 	foreach ($element->xpath('offers/offer') as $offer) {
				// 	// ob_end_clean();
				// 	ob_implicit_flush(1);
						
				// 		 echo "UPDATE xt_assortiment SET  product_limit = 100000, active = 1, price_opt_otpusk = ", $offer->price, ", price_mopt_otpusk = ", $offer->price, " WHERE id_supplier = ", 30939, " and sup_comment = '", $offer->vendorCode, "';<br />";
