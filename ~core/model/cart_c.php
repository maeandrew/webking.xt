...
class Cart {
	...
	// public function UpdateProduct($id_product = null, $opt, $box_qty, $qty, $sum, $note_opt = "",$note_mopt = "", $sum_default = null, $correction = null, $basic_price = null) {
	// 	if($correction == null){
	// 		$correction = $GLOBALS['CONFIG']['correction_set_0'];
	// 	}
	// 	$sum = round($sum, 2);
	...
	// 	$i = 3;
	// 	if($opt == 1){
	// 		if(!isset($_SESSION['Cart']['order_sum'][3]) || $_SESSION['Cart']['order_sum'][3]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[3],2)*$qty,2) < $GLOBALS['CONFIG']['retail_order_margin']){
	// 			$i = 3;
	// 		}elseif($_SESSION['Cart']['order_sum'][3]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[3],2)*$qty,2) >= $GLOBALS['CONFIG']['retail_order_margin'] && $_SESSION['Cart']['order_sum'][2]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[3],2)*$qty,2) < $GLOBALS['CONFIG']['wholesale_order_margin']){
	// 			$i = 2;
	// 		}elseif($_SESSION['Cart']['order_sum'][2]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[2],2)*$qty,2) >= $GLOBALS['CONFIG']['wholesale_order_margin'] && $_SESSION['Cart']['order_sum'][1]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[2],2)*$qty,2) < $GLOBALS['CONFIG']['full_wholesale_order_margin']){
	// 			$i = 1;
	// 		}elseif($_SESSION['Cart']['order_sum'][1]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[1],2)*$qty,2) >= $GLOBALS['CONFIG']['full_wholesale_order_margin']){
	// 			$i = 0;
	// 		}
	// 		$_SESSION['price_column'] = $i;
	// 		if((!isset($_SESSION['member']['gid']) || $_SESSION['member']['gid'] != 4) && isset($basic_price)){
	// 			$sum = round(round($basic_price*$correction[$i],2)*$qty,2);
	// 		}else{
	// 			if(isset($sum_default)){
	// 				$sum = $sum_default;
	// 			}else{
	// 				$sum = round(round($basic_price*$_SESSION['Cart']['personal_discount'], 2)*$qty, 2);
	// 			}
	// 		}
	// 		$_SESSION['Cart']['products'][$id_product]['order_opt_qty'] = $qty;
	// 		$_SESSION['Cart']['products'][$id_product]['order_box_qty'] = $box_qty;
	// 		$_SESSION['Cart']['products'][$id_product]['basic_opt_price'] = $basic_price;
	// 		$_SESSION['Cart']['products'][$id_product]['order_opt_sum'] = $sum;
	// 		$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default'] = $sum;
	// 		$_SESSION['Cart']['products'][$id_product]['opt_correction_set'] = $correction;
	// 		for($ii = 0; $ii < 4; $ii++){
	// 			$_SESSION['Cart']['products'][$id_product]['opt_sum_default'][$ii] = round(round($basic_price*$correction[$ii], 2)*$qty, 2);
	// 		}
	// 		if(null !== $sum_default){
	// 			$_SESSION['Cart']['products'][$id_product]['order_opt_sum'] = $sum_default;
	// 			$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default'] = $sum_default;
	// 		}
	// 	}else{
	// 		if(!isset($_SESSION['Cart']['order_sum'][3]) || $_SESSION['Cart']['order_sum'][3]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[3],2)*$qty,2) < $GLOBALS['CONFIG']['retail_order_margin']){
	// 			$i = 3;
	// 		}elseif($_SESSION['Cart']['order_sum'][3]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[3],2)*$qty,2) >= $GLOBALS['CONFIG']['retail_order_margin'] && $_SESSION['Cart']['order_sum'][2]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[2],2)*$qty,2) < $GLOBALS['CONFIG']['wholesale_order_margin']){
	// 			$i = 2;
	// 		}elseif($_SESSION['Cart']['order_sum'][2]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[2],2)*$qty,2) >= $GLOBALS['CONFIG']['wholesale_order_margin'] && $_SESSION['Cart']['order_sum'][1]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[1],2)*$qty,2) < $GLOBALS['CONFIG']['full_wholesale_order_margin']){
	// 			$i = 1;
	// 		}elseif($_SESSION['Cart']['order_sum'][1]-$_SESSION['Cart']['products'][$id_product]['order_opt_sum_default']+round(round($basic_price*$correction[1],2)*$qty,2) >= $GLOBALS['CONFIG']['full_wholesale_order_margin']){
	// 			$i = 0;
	// 		}
	...
	// 		$_SESSION['Cart']['products'][$id_product]['mopt_correction_set'] = $correction;
	// 		for($ii = 0; $ii < 4; $ii++){
	// 			$_SESSION['Cart']['products'][$id_product]['mopt_sum_default'][$ii] = round(round($basic_price*$correction[$ii],2)*$qty,2);
	// 		}
	// 		if(null !== $sum_default){
	...
	// }
	//*****************************************************************
	// public function SetSums() {
	// 	$_SESSION['Cart']['order_opt_sum']				= 0;
	// 	$_SESSION['Cart']['order_mopt_sum']				= 0;
	// 	$_SESSION['Cart']['order_opt_sum_default']		= 0;
	// 	$_SESSION['Cart']['order_mopt_sum_default']		= 0;
	// 	$_SESSION['Cart']['order_opt_sum_discount']		= 0;
	// 	$_SESSION['Cart']['order_mopt_sum_discount']	= 0;
	// 	$_SESSION['Cart']['opt_sum_default']			= array(0,0,0,0);
	// 	$_SESSION['Cart']['mopt_sum_default']			= array(0,0,0,0);
	// 	if(isset($_SESSION['Cart']['products'])){
	// 		// Вычисление общих сумм опт и мопт
	// 		foreach($_SESSION['Cart']['products'] as $p){
	// 			if(isset($_SESSION['price_mode']) && $_SESSION['price_mode'] == 1){
	// 				if($p['basic_opt_price'] > 0 && !empty($p['opt_correction_set'])){
	// 					$_SESSION['Cart']['order_opt_sum']	+= round(round($p['basic_opt_price']*$_SESSION['Cart']['personal_discount'],2)*$p['order_opt_qty'],2);
	// 				}
	// 				if($p['basic_mopt_price'] > 0 && !empty($p['mopt_correction_set'])){
	// 					$_SESSION['Cart']['order_mopt_sum']	+= round(round($p['basic_mopt_price']*$_SESSION['Cart']['personal_discount'],2)*$p['order_mopt_qty'],2);
	// 				}
	// 			}else{
	// 				if($p['basic_opt_price'] > 0 && !empty($p['opt_correction_set'])){
	// 					$_SESSION['Cart']['order_opt_sum']	+= round(round($p['basic_opt_price']*$p['opt_correction_set'][isset($_SESSION['price_column'])?$_SESSION['price_column']:3], 2)*$p['order_opt_qty'], 2);
	// 				}
	// 				if($p['basic_mopt_price'] > 0 && !empty($p['mopt_correction_set'])){
	// 					$_SESSION['Cart']['order_mopt_sum']	+= round(round($p['basic_mopt_price']*$p['mopt_correction_set'][isset($_SESSION['price_column'])?$_SESSION['price_column']:3], 2)*$p['order_mopt_qty'], 2);
	// 				}
	// 			}
	// 			isset($p['order_opt_sum_default'])	? $_SESSION['Cart']['order_opt_sum_default'] += round($p['order_opt_sum_default'], 2)	: null;
	// 			isset($p['order_mopt_sum_default'])	? $_SESSION['Cart']['order_mopt_sum_default'] += round($p['order_mopt_sum_default'], 2)	: null;
	// 			for($i = 0; $i < 4; $i++){
	// 				isset($p['opt_sum_default'][$i])	? $_SESSION['Cart']['opt_sum_default'][$i] += round($p['opt_sum_default'][$i], 2)	: null;
	// 				isset($p['mopt_sum_default'][$i])	? $_SESSION['Cart']['mopt_sum_default'][$i] += round($p['mopt_sum_default'][$i], 2)	: null;
	// 			}
	// 		}
	// 		// Начисление скидки/наценки
	// 		if(isset($_SESSION['Cart']['discount'])){
	// 			$_SESSION['Cart']['order_mopt_sum_discount']	= $_SESSION['Cart']['order_mopt_sum'];
	// 			$_SESSION['Cart']['order_opt_sum_discount']		= $_SESSION['Cart']['order_opt_sum'];
	// 		}
	// 	}
	// }
	//********************************************************
	// public function SetAllSums() {
	// 	$this->SetSums();
	// 	$cart_sum = round($_SESSION['Cart']['order_opt_sum'],2) + round($_SESSION['Cart']['order_mopt_sum'],2);
	// 	$cart_sum_default = round($_SESSION['Cart']['order_opt_sum_default'],2) + round($_SESSION['Cart']['order_mopt_sum_default'],2);
	// 	$_SESSION['Cart']['sum_default'] = $cart_sum_default;
	// 	for($i = 0; $i < 4; $i++){
	// 		$_SESSION['Cart']['order_sum'][$i] = round($_SESSION['Cart']['opt_sum_default'][$i],2) + round($_SESSION['Cart']['mopt_sum_default'][$i],2);
	// 	}
	// 	$_SESSION['Cart']['sum'] = $cart_sum;
	// 	if(!isset($_SESSION['Cart']['order_sum'][3]) || $_SESSION['Cart']['order_sum'][3] < $GLOBALS['CONFIG']['retail_order_margin']){
	// 		$_SESSION['price_column'] = 3;
	// 	}elseif($_SESSION['Cart']['order_sum'][3] >= $GLOBALS['CONFIG']['retail_order_margin'] && $_SESSION['Cart']['order_sum'][2] < $GLOBALS['CONFIG']['wholesale_order_margin']){
	// 		$_SESSION['price_column'] = 2;
	// 	}elseif($_SESSION['Cart']['order_sum'][2] >= $GLOBALS['CONFIG']['wholesale_order_margin'] && $_SESSION['Cart']['order_sum'][1] < $GLOBALS['CONFIG']['full_wholesale_order_margin']){
	// 		$_SESSION['price_column'] = 1;
	// 	}elseif($_SESSION['Cart']['order_sum'][1] >= $GLOBALS['CONFIG']['full_wholesale_order_margin']){
	// 		$_SESSION['price_column'] = 0;
	// 	}
	// 	if(isset($_SESSION['Cart']['sum'])){
	// 		$_SESSION['Cart']['sum_discount'] = round($_SESSION['Cart']['order_opt_sum_discount'],2) + round($_SESSION['Cart']['order_mopt_sum_discount'],2);
	// 	}else{
	// 		$_SESSION['Cart']['sum_discount'] = 0;
	// 	}
	// }
	/***********************************************************/
	...
	// }
	/***********************************************************/
	// public function SetPersonalDiscount($discount){
	// 	$_SESSION['Cart']['personal_discount'] = 1-$discount/100;
	// }
	/***********************************************************/
	// public function SetSumDiscount(){
	// 	$discount = 1;
	// 	if(isset($_SESSION['price_mode']) && $_SESSION['price_mode'] == 1){
	// 		$discount = 1;
	// 	}else{
	// 		if(isset($_SESSION['Cart']['personal_discount'])){
	// 			$discount = $_SESSION['Cart']['personal_discount'];
	// 		}else{
	// 			$discount = 1;
	// 		}
	// 	}
	...
	// 	if($_SESSION['Cart']['prod_qty'] > 0){
	// 		$word = (string)G::WordForNum($_SESSION['Cart']['prod_qty']);
	// 		$string = $_SESSION['Cart']['prod_qty']." $word на ".number_format($_SESSION['Cart']['sum_discount'],2,",","")." грн.";
	// 		return $string;
	// 	}else{
	// 		return "";
	// 	}
	...
	// Заполняет корзину товарами, такими как в заказе $id_order
	// public function FillByOrderId2($id_order, $add = null){
	// 	$Order = new Orders();
	// 	$Customer = new Customers();
	// 	$Order->SetFieldsById($id_order);
	// 	$order = $Order->fields;
	...
	// 	}
	// 	foreach($products as $p){
	// 		$Products->SetFieldsById($p['id_product']);
	// 		$product = $Products->fields;
	// 		if(isset($_SESSION['price_mode']) && $_SESSION['price_mode'] == 1){
	// 			if($_SESSION['client']['user_agent'] == 'desktop'){
	// 				if($p['opt_qty'] > 0){
	// 					if($this->IsAvailableProduct2($p['id_product'])){
	// 						$this->UpdateProduct($p['id_product'], 1, $p['box_qty'], $p['opt_qty'], round($p['opt_sum'] * $personal_discount, 2), $p['note_opt'], $p['note_mopt'], round($p['default_sum_opt'] * $personal_discount, 2), explode(';', $GLOBALS['CONFIG']['correction_set_'.$product['opt_correction_set']]), $product['price_opt']);
	// 					}else{
	// 						$_SESSION['Cart']['unavailable_products'][] = $p;
	// 					}
	...
	// 			if($p['mopt_qty'] > 0){
	// 				if($this->IsAvailableProduct2($p['id_product'])){
	// 					$this->UpdateProduct($p['id_product'], 0, null, $p['mopt_qty'], round($p['mopt_sum'] * $personal_discount, 2), $p['note_opt'], $p['note_mopt'], round($p['default_sum_mopt'] * $personal_discount, 2), explode(';', $GLOBALS['CONFIG']['correction_set_'.$product['mopt_correction_set']]), $product['price_mopt']);
	// 				}else{
	// 					$_SESSION['Cart']['unavailable_products'][] = $p;
	// 				}
	// 			}
	...
	// }
	// Проверка на доступность товара
	// public function IsAvailableProduct2($id_product){
	// 	global $db;
	// 	$sql = "SELECT p.id_product
	// 		FROM "._DB_PREFIX_."product p
	// 		WHERE p.id_product = ".$id_product."
	...
	// 		AND (p.price_mopt > 0 OR p.price_opt > 0)";
	// 	// Добавить если нужно отсекать товары на которые нет поставщиков
	// 	$this->list = $db->GetOneRowArray($sql);
	// 	if(!$this->list){
	// 		return false;
	// 	}
	// 	return true;
