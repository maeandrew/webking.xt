		...
		foreach($this->pages as $key=>$title){
			if(is_numeric($key)){														// для исключения пред. и следующ
				if($this->cur_page_id != $title){
					...
					$name[] = '<li class="page'.$title.'"><a href="'.$url.'" class="animate paper_shadow_0 bg-white color-grey">'.$title.'</a></li>';
				}else{
					$name[] = '<li class="page'.$title.' active"><a class="bg-white color-grey">'.$title.'</a></li>';
				}
			}else{
				if($title == '/p1/'){
					$this->pages[$key] = '/';
