	...
	*/
	...
	public function GetCatSegmentation($id_segm){
		$sql = 'SELECT id_category FROM '._DB_PREFIX_.'category WHERE id_category IN
			...
			LEFT JOIN '._DB_PREFIX_.'assortiment a ON a.id_product = sp.id_product
			LEFT JOIN '._DB_PREFIX_.'product p ON p.id_product = sp.id_product
			WHERE (p.price_opt>0 OR p.price_mopt>0) AND sp.id_segment IN
			(SELECT id FROM '._DB_PREFIX_.'segmentation WHERE id = '.$id_segm.')
			AND a.product_limit > 0 AND a.active = 1)) AND visible > 0';
		$res = $this->db->GetArray($sql);
