			...
			case "segments":
				...
				if($_POST['type'] == 1 || $_POST['type'] == 2) {
					...
					foreach ($segments as &$v) {
						$segm .= '<li data-id="' . $v['id'] . '" onclick="segmentOpen(' . $v['id'] . ')"> <span class="link_wrap"><a href="#">' . $v['name'] . '</a><span><i class="material-icons">&#xE315;</i></span></span> </li>';
					}
					$segm .= '</ul>';
					echo $segm;
					exit();
					...
					$cat = $Products->generateNavigation($navigation);
					echo $cat;
					exit();
				}
				break;
			case "segmid":
				echo $Products->generateNavigation($Products->navigation($dbtree->GetCatSegmentation($_POST['idsegment'])));
				exit();
				break;
			default:
				break;
