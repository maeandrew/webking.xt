<?php
	...
	$file_exsel = '/var/www/xt.ua/web/post_images/S08SH.xlsx';
		...
		//получаем имена фото в указаной папке
	 	$PATH_product_images = '/var/www/xt.ua/web/post_images/S08/';	
		...
		$offer_add = array();
		//Товары с фотографий
		echo "Товары с фотографий************************************************************************************* <br/>";
		foreach ($array as $key => $value) {
			if (array_key_exists(strval ($value['0']), $array_img) && !in_array($value[0], $supcomments)) {
			// if (array_key_exists(strval ($value['0']), $array_img)) {
				$offer_add[] = $value;
				echo $i++, ';+;', $value['0'], ';', $value['1'], ';', $value['2'], ';', $value['3'], ';', $value['4'], "<br/>";
			}
			
		}
		//Товары без фотографий
		echo "Товары без фотографий************************************************************************************* <br/>";
		$i2 = 0;
		foreach ($array as $key => $value) {
			// if (array_key_exists(strval ($value['0']), $array_img) && !in_array($value[0], $supcomments)) {
			if (!array_key_exists(strval ($value['0']), $array_img)) {
				$offer_add[] = $value;
				echo $i2++, ';', $value['0'], ';', $value['1'], ';', $value['2'], ';', $value['3'], "<br/>";
			}
	...
	$skipped = false;
	if(!$product = $Parser->s08cvet($offer, $array_img)){
		continue;
	}
	
	//выбераем нашу категорию
		echo 'id_category -> ',$id_category = $offer[3], "<br />";			
