<?php
    ...
    $filename = '/var/www/xt.ua/web/post_images/arrayX43_br';    
    // Чтение.
    $data = file_get_contents($filename);
    $array_br = unserialize($data);
    var_dump($array_br);
    //можем обнулить файл брака
    // $array_br = array('100', '101');
    // $data = serialize($array_br);      // PHP формат сохраняемого значения.
    // file_put_contents($filename, $data);
    // die();
    //Посмотрим результаты обработки файла          
    echo 'Сортируем на обнавление и добавление<br/>';
    $file = '/var/www/xt.ua/web/post_images/arrayX43';  
        ...
        if (!$array_value) {
            ...
            foreach ($array_value as $key => $offer) {
                if (in_array(trim($offer['0']), $supcomments)) {
                    ...
                    if (!in_array(trim($offer['0']), $array_br)){
...
// die();
    ...
    echo "***********************СТАРТ******************************<br />";
    if(!$product = $Parser->elfa_x43($offer)){
        ...
        if($id_product = $Products->AddProduct($product)){
            ...
            if(isset($product['images']) && !empty($product['images'])){
                foreach($product['images'] as $key=>$image){
                    $to_resize[] = $newname = $article['art'].($key == 0?'':'-'.$key).substr($image, -4);
